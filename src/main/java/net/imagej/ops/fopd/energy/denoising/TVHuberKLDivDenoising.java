...
public class TVHuberKLDivDenoising<T extends RealType<T>> extends AbstractDenoising<T> {
	...
	CostFunction<T> getCostFunction(RandomAccessibleInterval<T>[] input, LinearOperator<T>[] ascentOperator,
		...
		return new KLDivergence2D<T>(ops, input, ascentOperator, descentOperator, (1.0 / (4.0 + input.length)));
	}
}
