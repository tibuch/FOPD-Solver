...
public class TVHuberSquaredL2Deconvolution<T extends RealType<T>> extends AbstractDeconvoltuion<T> {
	...
	CostFunction<T> getCostFunction(RandomAccessibleInterval<T>[] input, LinearOperator<T>[] ascentConvolver,
		...
		return new SquaredL2Norm2D<T>(ops, input, ascentConvolver, descentConvolver, (1.0 / (4.0 + input.length)));
	}
}
