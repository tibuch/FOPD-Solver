...
public class CostFunctionTests extends AbstractOpTest {
	...
	private static double[] expectedSquaredL2Norm = new double[] { 0.0625, 0.0625, 0.0625, 0.0625, 0.0, 0.0625, 0.0625,
			0.0625, 0.0625 };
	...
	public void l1NormTest() {
		...
		int i = 0;
		while (c.hasNext()) {
			assertEquals("L1-Norm differs", expectedL1Norm[i++], c.next().get(), 0);
		}
	}
	...
	public void klDivTest() {
		...
		int i = 0;
		while (c.hasNext()) {
			assertEquals("Kullback-Leibler-Divergence differs", expectedKLDiv[i++], c.next().get(), 0);
		}
	}
	...
	public void squaredL2NormTest() {
		...
		final SquaredL2Norm2DAscent<DoubleType> ascentL1Denoising = ops.op(SquaredL2Norm2DAscent.class,
				SolverState.class, img, ops.op(Identity.class, img));
		final SquaredL2Norm2DDescent<DoubleType> descentL1Denoising = ops.op(SquaredL2Norm2DDescent.class,
				SolverState.class, ops.op(Identity.class, img), 0.25);
		...
		int i = 0;
		while (c.hasNext()) {
			assertEquals("Squared L2-Norm differs", expectedSquaredL2Norm[i++], c.next().get(), 0);
		}
	}
}
